import 'package:flutter/material.dart';

const tPrimaryColor = Color(0xFF4CAF50);
const tSecondarycolor = Color(0xFF272727);
const tAccentColor = Color(0xFF001BFF);

const tWhatsappNavColor = Color(0xFF1E292F);
const tWhatsappBgColor = Color(0xFF101920);

const tCardBgColor = Color(0xFFF7F6F1);
const tWhiteColor = Color(0xFFFFFFFF);
const tDarkColor = Color(0xFF000000);

// on boarding colors
const tOnBoardingPage1Color = Colors.white;
const tOnBoardingPage2Color = Color(0xFFFDDCDF);
const tOnBoardingPage3Color = Color(0xFFFFDCBD);

// Figma
const tBodyTextColor = Color(0xFF282828);
const tCategoryButtonBg = Color(0xFFF0F0F0);
const tSearchBarBg = Color(0xFFF5F5F5);
